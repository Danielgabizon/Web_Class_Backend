...
beforeAll(async () => {
    ...
    await postModel.deleteMany();
       ...
       const response3 = await request(app).post("/posts").set({ authorization: "jwt " + user.accessToken }).send(testPost[0]);
        const response4 = await request(app).post("/posts").set({ authorization: "jwt " + user.accessToken }).send(testPost[1]);
